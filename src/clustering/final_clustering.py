...
class DataPreprocessor:
    ...
    def label_encode_column(self, col):
        ...
        self.mapping_dicts[col] = dict(enumerate(le.classes_))
    ...
    def one_hot_encode_column(self, col):
        ...
        """
        self.categorical_columns_hot_encoded.append(col)
        ohe = OneHotEncoder(sparse_output=False)
        encoded_features = ohe.fit_transform(self.df[[col]])
        encoded_df = pd.DataFrame(encoded_features, columns=ohe.get_feature_names_out([col]))
        self.df = self.drop_and_concat(self.df, encoded_df, col)
    ...
    def drop_and_concat(original_df, new_cols_df, col_to_drop):
        ...
        """
        original_df = original_df.drop([col_to_drop], axis=1)
        return pd.concat([original_df, new_cols_df], axis=1)
    ...
    def prepare_final_df(self):
        ...
        """
        return self.df.drop(["userid"], axis=1).copy()
...
class Clusterer:
    ...
    def find_optimal_clusters(self, max_k=10):
        ...
        silhouette_scores = []
        for k in iters:
            gmm_model = GaussianMixture(n_components=k, covariance_type='full', random_state=0).fit(self.df)
            cluster_labels = gmm_model.predict(self.df)
            silhouette_scores.append(silhouette_score(self.df, cluster_labels, metric="euclidean"))
    ...
    def apply_gmm_clustering(self):
        ...
        """
        self.gmm = GaussianMixture(n_components=self.optimal_clusters, covariance_type='full', random_state=0).fit(self.df)
        self.final_df = self.df.copy()
        self.final_df["cluster"] = self.gmm.predict(self.df)
        self.final_df["userid"] = self.moodle_ids
...
class FeatureIdentifier:
    ...
    def prepare_data(self, i):
        ...
        """
        target_col = f"Cluster_{i}"
        classification_df = self.df.copy()
        classification_df = classification_df.drop("userid", axis=1)
        classification_df[target_col] = (classification_df["cluster"] == i).astype(int)
        X = classification_df.drop(["cluster", target_col], axis=1)
    ...
    def train_and_evaluate_model(X_train, y_train, X, y):
        ...
        cv_f1_scores = cross_val_score(model, X, y, cv=10, scoring="f1_macro")
        cv_accuracy_scores = cross_val_score(model, X, y, cv=10, scoring="accuracy")
        return cv_f1_scores.mean(), cv_accuracy_scores.mean(), model.feature_importances_
    ...
    def identify_importance(self):
        ...
        for i in range(self.optimal_clusters):
            X_train, X_test, y_train, y_test = self.prepare_data(i)
            f1_score, accuracy, feature_importances = self.train_and_evaluate_model(X_train, y_train, pd.concat([X_train, X_test], ignore_index=True), pd.concat([y_train, y_test], ignore_index=True))
            feature_names_list = X_train.columns.tolist()

            self.f1_score_dict[f"cluster_{i}"] = f1_score
            self.accuracy_dict[f"cluster_{i}"] = accuracy
            self.feature_importance_dict[f"cluster_{i}"] = dict(zip(feature_names_list, feature_importances))
    ...
    def filter_dataset(self, threshold=0.05):
        ...
        for i in range(self.optimal_clusters):
            ...
            cluster_features = {feature for feature, importance in feature_importances.items() if importance > threshold}
    ...
    def display_f1_scores(self):
        ...
        for cluster, f1 in sorted(self.f1_score_dict.items(), key=lambda x: x[1], reverse=True):
    ...
    def display_accuracy_scores(self):
        ...
        for cluster, accuracy in sorted(self.accuracy_dict.items(), key=lambda x: x[1], reverse=True):
    ...
    def save_feature_importance_to_csv(self, data_preprocessor, save_path):
        ...
        # Check if the 'Top_Level_Feature' column is created correctly
        feature_importance_df_["Top_Level_Feature"] = feature_importance_df_["Feature"].str.split("_").str[0]

        if "Top_Level_Feature" not in feature_importance_df_.columns:
            raise KeyError("Top_Level_Feature column was not created correctly.")

        # Group by 'Cluster' and 'Top_Level_Feature' and then calculate the mean importance.
        mean_feature_importance_df = (
            feature_importance_df_.groupby(["Cluster", "Top_Level_Feature"])["Importance"]
            .mean()
            .reset_index()
        )
        ...
        mean_feature_importance_df = mean_feature_importance_df[
            mean_feature_importance_df["Feature"].isin(data_preprocessor.categorical_columns_hot_encoded)
        ]
        ...
        # Cluster as integer
        feature_importance_df_["Cluster"] = feature_importance_df_["Cluster"].astype(int)
...
class DecisionTree:
    ...
    def recurse_extract_rules(self, node, rules):
        ...
        feature_names = self.df.drop("cluster", axis=1).columns.tolist()  # Get the feature names
        ...
        if tree_.feature[node] != -2:
            ...
            self.recurse_extract_rules(tree_.children_left[node], rules + [f"{name} <= {threshold}"])  # Recurse
            self.recurse_extract_rules(tree_.children_right[node], rules + [f"{name} > {threshold}"])  # Recurse
        # If the current node is a leaf node, extract the rules
        else:
            leaf_cluster = tree_.value[node].argmax()  # Get the leaf cluster
            self.leaf_cluster_map[node] = leaf_cluster  # Map the leaf node to the leaf cluster
            self.all_rules[node] = rules  # Store the rules
            self.cluster_all_rules.setdefault(leaf_cluster, []).append(rules)  # Store the rules for the leaf cluster
        ...
        for rule in rules:
            if isinstance(rule, str):  # Check if 'rule' is a string
                ...
                if operator == "<=":  # Check the operator
                    ...
                    temp_df = temp_df[temp_df[feature] > float(value)]  # Apply the rule
        return [self.mapping_dicts.get(target_column, {}).get(code, code)  # Get the mapped value
...
class Visualizer:
    ...
    def plot_pca(self, save_path):
        ...
        principal_df.to_csv(Path.cwd() / "data/clustering/pca2.csv", index=False)
...
class SaveData:
    ...
    def merge_original_data(self, data_preprocessor, decision_tree):
        ...
        self.df = pd.merge(
            ...
            decision_tree.final_df[["userid", "cluster", "rule_index"]],
            on="userid",
            how="right",
        )
...
class DataHandler:
    ...
    def run_feature_identification(clusterer, data_preprocessor):
        ...
        feature_identifier.save_feature_importance_to_csv(
            data_preprocessor,
            Path.cwd() / "data/clustering/feature_importance2.csv",
        )
    ...
    def run_decision_tree(
        ...
        decision_tree.save_cluster_variables(
            data_preprocessor,
            feature_identifier,
            clustering_data_path / "cluster_variables2.json",
        )
        decision_tree.save_rules_to_csv(clustering_data_path / "rules2.csv")
        decision_tree.plot_tree(clustering_data_path / "images/decision_tree2.svg")
    ...
    def run_visualization(
        ...
        visualizer.plot_pca(clustering_data_path / "images/pca2.svg")
        return visualizer
...
def main():
    ...
    # Run the pipeline with dataset 1
    handler = DataHandler(clustering_data_path / "clustering_dataset2.csv")
    ...
    feature_identifier = handler.run_feature_identification(clusterer, data_preprocessor)
    decision_tree = handler.run_decision_tree(feature_identifier.final_df, data_preprocessor.mapping_dicts, data_preprocessor, feature_identifier, clusterer)
    handler.run_visualization(clusterer.final_df)

    # Merge original data with clustering results
    merged_df = handler.merge_dataframes(data_preprocessor.original_df, decision_tree.final_df)
